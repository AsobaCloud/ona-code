...
async function runOpenAICompatModelLoop(db, rt, io, hookRt, provider, model) {
  ...
  for (;;) {
    ...
    const toolUses = assistantBlocks.filter(b => b.type === 'tool_use')
    if (!toolUses.length) {
      // Render assistant text with markdown
...
async function executeToolUses(db, rt, io, hookRt, toolUses) {
  for (const tu of toolUses) {
    ...
    io.println(ui.formatToolStart(toolName))
    ...
    if (!pre.ok || pre.preTool?.denied) {
      ...
      io.println(ui.formatToolResult(toolName, msg, true))
      continue
    }
    ...
    if (decision === 'deny') {
      ...
      io.println(ui.formatToolResult(toolName, 'Permission denied by policy.', true))
      continue
    }
    ...
    io.println(ui.formatToolResult(toolName, out.content, out.is_error))

...
import { resolveAnthropicCredentials, resolveZhipuApiKey } from './auth.mjs'
...
# System
 ...
 - When a tool returns an error or no results (e.g. Read: not found, Glob: no matches), you MUST report the failure to the user honestly. Do NOT assume the tool succeeded or generate a response as if it did. Ask the user for clarification or the correct path.
...
export async function runUserTurn(db, rt, userText, io) {
  ...
  if (provider === 'lm_studio_local' || provider === 'openai_compatible' || provider === 'zhipu' || provider === 'ollama') {
...
async function runOpenAICompatModelLoop(db, rt, io, hookRt, provider, model, systemPrompt) {
  ...
  if (provider === 'lm_studio_local') {
    ...
    apiKey = resolveZhipuApiKey()
    if (!nonempty(apiKey)) { io.println('zhipu requires a ZAI API key. Run /login to save one.'); return }
  } else if (provider === 'ollama') {
    baseUrl = 'http://192.168.5.238:11435/v1'
    apiKey = 'ollama' // typically ignored
  } else {
  ...
  for (;;) {
    ...
    try {
      ...
      // Tool labels are displayed inside executeToolUses, after permission check — not here
    } catch (e) {
      await runHooks(db, hookRt, 'StopFailure', { error: classifyError(e), error_details: e.message })
      io.println(`\n[model error] ${e.message}`)
      return
    ...
    if (!toolUses.length) {
      ...
      // Text was already streamed live via io.write — only render via io.println
      // if the UI uses a stream buffer (Ink), in which case we commit it as an assistant message.
      // Do NOT call io.println here — that would duplicate the streamed output.
      // The Ink UI handles this via app.addAssistantMessage in bin/agent.mjs after runUserTurn returns.
      await runHooks(db, hookRt, 'Stop', { stop_hook_active: false, last_assistant_message: fullText })
      break
    }
    await executeToolUses(db, rt, io, hookRt, toolUses)
    
    // Restart spinner after tool execution for continued model processing
    if (io.spinner) io.spinner.start('Thinking')
  }
}
...
async function executeToolUses(db, rt, io, hookRt, toolUses) {
  ...
  for (const tu of toolUses) {
    ...
    // Show tool label only after permission confirmed
    ...
    // §4 Bug fix: propagate CWD change from Bash back to rt so subsequent tools use updated path

...
export function transcriptToAnthropicMessages(db, sessionId) {
  // Check for collapse_commit — if present, start from there
  const collapse = db.prepare(
    `SELECT sequence, payload_json FROM transcript_entries WHERE session_id = ? AND entry_type = 'collapse_commit' ORDER BY sequence DESC LIMIT 1`
  ).get(sessionId)

  let rows
  if (collapse) {
    const summary = JSON.parse(collapse.payload_json).summary || ''
    rows = [
      { entry_type: 'user', payload_json: JSON.stringify({ _t: 'user', content: [{ type: 'text', text: `[Previous conversation summary]\n${summary}` }] }) },
      ...db.prepare(
      ...
      ).all(sessionId, collapse.sequence),
    ]
  } else {
    rows = db.prepare(
...
/**
 * Epistemic isolation filter — §8.5.1.
 *
 * Strips SEARCH/REPLACE implementation blocks from assistant messages before
 * building the context for the TEST phase. This guarantees the test-generation
 * agent cannot read the implementation it is supposed to independently verify.
 *
 * A block is defined as any sequence matching:
 *   <<<<<<< SEARCH ... ======= ... >>>>>>> REPLACE
 * or the equivalent file-path-prefixed form used by Nehanda.
 *
 * Given: an array of OpenAI-format messages and the active workflow phase
 * When: phase === 'test'
 * Then: every assistant message has SEARCH/REPLACE blocks replaced with a
 *       redaction placeholder; all other messages pass through unchanged.
 * When: phase !== 'test'
 * Then: messages are returned unmodified.
 */
export function applyEpistemicFilter(messages, phase) {
  if (phase !== 'test') return messages

  const SEARCH_REPLACE_RE = /<<<<<<< SEARCH[\s\S]*?>>>>>>> REPLACE/g
  const PLACEHOLDER = '[implementation details redacted for epistemic isolation]'
  ...
  return messages.map(msg => {
    ...
    if (typeof msg.content === 'string') {
      const filtered = msg.content.replace(SEARCH_REPLACE_RE, PLACEHOLDER)
      return filtered === msg.content ? msg : { ...msg, content: filtered }
    }

    // Array content (text blocks + possible tool_calls)
    if (Array.isArray(msg.content)) {
      const filtered = msg.content.map(block => {
        if (typeof block === 'string') {
          return block.replace(SEARCH_REPLACE_RE, PLACEHOLDER)
        }
        if (block?.type === 'text' && typeof block.text === 'string') {
          const t = block.text.replace(SEARCH_REPLACE_RE, PLACEHOLDER)
          return t === block.text ? block : { ...block, text: t }
        }
        return block
      })
      // Only return a new object if something actually changed
      const changed = filtered.some((b, i) => b !== msg.content[i])

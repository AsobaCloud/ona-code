...
/** §5.2 — dedup: collapse adjacent identical (hook_event, matcher, command, shell, if_condition) keeping first. */
function hooksForEvent(settings, eventName) {
  const raw = settings?.hooks
  if (!Array.isArray(raw)) return []
  const filtered = raw.filter(h => h && h.hook_event_name === eventName)
  const deduped = []
  for (const h of filtered) {
    const key = `${h.hook_event_name}|${h.matcher ?? ''}|${h.command ?? ''}|${h.shell ?? 'bash'}|${h.if_condition ?? ''}`
    if (deduped.length && deduped[deduped.length - 1]._dedupKey === key) continue
    deduped.push({ ...h, _dedupKey: key })
  }
  return deduped
}
...
async function runHookCommand(shell, commandString, stdinObj, cwd, runtimeDbPath, timeoutMs) {
  ...
  const env = { ...process.env, AGENT_SDLC_DB: runtimeDbPath, SDLC_HOOK: '1', CLAUDE_PROJECT_DIR: cwd }

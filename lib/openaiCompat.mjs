...
export async function streamOpenAIChatCompletion({ baseUrl, apiKey, model, messages, tools, io, maxTokens = 8192, bufferOutput = false }) {
  ...
  while (true) {
    ...
    for (const line of parts) {
      ...
      if (delta?.content) {
        ...
        // Only stream to output if not buffering (Task 4.4)
        if (!bufferOutput) {
  ...
  return { 
    ...
    bufferedText: bufferOutput ? fullText : null // Return buffered text if buffering enabled (Task 4.4)
  }
}

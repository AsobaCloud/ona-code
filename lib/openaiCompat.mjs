...
export async function streamOpenAIChatCompletion({ baseUrl, apiKey, model, messages, tools, io, maxTokens = 8192 }) {
  ...
  while (true) {
    ...
    for (const line of parts) {
      ...
      if (delta?.content) { fullText += delta.content; io.write(delta.content) }
      if (delta?.tool_calls) {
  ...
  const toolCalls = [...toolAcc.keys()].sort((a, b) => a - b).map(idx => {
    ...
    let input = {}
    try { input = acc.arguments.trim() ? JSON.parse(acc.arguments) : {} } catch { input = { _raw: acc.arguments } }

...
// ── Clawd mascot — from claude-code/components/LogoV2/Clawd.tsx ──
// Apple Terminal variant (simpler block chars)
function Clawd() {
  ...
  // Standard terminal — block chars with color
  return e(Box, { flexDirection: 'column', alignItems: 'center' },
    e(Text, {},
      e(Text, { color: '#cc785c' }, ' \u2590'),
      e(Text, { color: '#cc785c', backgroundColor: '#3d2b1f' }, '\u259B\u2588\u2588\u2588\u259C'),
      e(Text, { color: '#cc785c' }, '\u258C'),
    ),
    e(Text, {},
      e(Text, { color: '#cc785c' }, '\u259D\u259C'),
...
// Two-column bordered box matching Claude Code's layout
function WelcomeBanner({ version, provider, wireModel, cwd, permissionMode, bare }) {
  let cols = 80
  try { cols = process.stdout.columns || 80 } catch {}

  const borderTitle = ` ona v${version} `
  const modelLine = `${wireModel}${bare ? ' [bare]' : ''}`
  const cwdDisplay = cwd.startsWith(process.env.HOME || '~')
    ? '~' + cwd.slice((process.env.HOME || '~').length)
    : cwd

  // Narrow terminals: single column
  if (cols < 60) {
    return e(Box, {
      flexDirection: 'column', borderStyle: 'round', borderColor: '#cc785c',
      width: cols, paddingX: 1, paddingY: 1, alignItems: 'center',
    },
      e(Text, { bold: true }, 'Welcome to ona!'),
      e(Clawd, {}),
  ...
  const leftWidth = Math.min(50, Math.floor(cols * 0.45))
  const rightWidth = cols - leftWidth - 5 // borders + padding + separator

  return e(Box, {
    flexDirection: 'column', borderStyle: 'round', borderColor: '#cc785c',
    width: cols,
  },
    e(Box, { flexDirection: 'row', paddingX: 1, gap: 1 },
      // Left panel: welcome + mascot + model info
      e(Box, { flexDirection: 'column', width: leftWidth, justifyContent: 'space-between', alignItems: 'center', minHeight: 9 },
        e(Box, { marginTop: 1 }, e(Text, { bold: true }, 'Welcome to ona!')),
        e(Clawd, {}),
        e(Box, { flexDirection: 'column', alignItems: 'center' },
          e(Text, { dimColor: true }, modelLine),
          e(Text, { dimColor: true }, cwdDisplay),
        ),
      ),
      // Separator
      e(Box, { height: '100%', borderStyle: 'single', borderColor: '#cc785c', borderDimColor: true,
        borderTop: false, borderBottom: false, borderLeft: false }),
      // Right panel: tips
      e(Box, { flexDirection: 'column', width: rightWidth, paddingLeft: 1 },
        e(Text, { bold: true }, 'Tips for getting started'),
        e(Text, { dimColor: true }, 'Type a message to start coding with ona'),
        e(Text, { dimColor: true }, 'Use /help to see available commands'),
        e(Text, { dimColor: true }, 'Use /model to switch models'),
        e(Box, { marginTop: 1 },
          e(Text, { dimColor: true }, '\u2500'.repeat(Math.max(10, rightWidth - 2))),
        ),
        e(Text, { bold: true }, 'Provider'),
        e(Text, { dimColor: true }, provider),
      ),
    ),
  )
}
...
function Footer({ phase, model, permissionMode }) {
  return e(Box, { marginLeft: 1 },
    e(Text, { dimColor: true }, `${phase || 'idle'} ${BULLET} ${model || ''} ${BULLET} ${permissionMode || 'default'} ${BULLET} ? for shortcuts`),
  )
}
...
export function startApp(config) {
  ...
  return {
    ...
    askUser(question) { return new Promise(resolve => { bridge.setAskState?.({ question, resolve }) }) },
    exit() { bridge.exit?.() }
  }
}

...
export async function executeBuiltinTool(db, ctx, toolName, toolInput, io) {
  ...
  // §1 Bug fix: surface JSON parse failures from streaming tool call arguments
  if (toolInput?._parseError) {
    const raw = String(toolInput._raw || '').slice(0, 200)
    return { content: `Tool call arguments could not be parsed (truncated JSON): ${raw}`, is_error: true }
  }
  ...
  try {
    switch (toolName) {
      ...
      case 'SubmitImplementation': return await toolSubmitImplementation(db, ctx, io)
      case 'EnterPlanMode': return toolEnterPlanMode(db, ctx)
      case 'ExitPlanMode': return await toolExitPlanMode(db, ctx, toolInput, io)
      case 'Agent': return await toolAgent(db, ctx, toolInput, io)
      case 'Skill': return toolSkill(cwd, toolInput)
...
function toolWrite(cwd, input, db, ctx) {
  ...
  const abs = path.isAbsolute(rel) ? path.resolve(rel) : path.resolve(cwd, rel)
  if (abs !== path.resolve(cwd) && !abs.startsWith(path.resolve(cwd) + path.sep)) {
...
function toolEdit(cwd, input, db, ctx) {
  ...
  const abs = path.isAbsolute(rel) ? path.resolve(rel) : path.resolve(cwd, rel)
  if (abs !== path.resolve(cwd) && !abs.startsWith(path.resolve(cwd) + path.sep)) {
    return { content: 'Edit: path outside working directory', is_error: true }
  }
...
async function toolBash(cwd, input, db, ctx) {
  ...
  return new Promise(resolve => {
    const child = spawn(resolveBash(), ['-c', wrappedCmd], { cwd, env: { ...process.env }, stdio: ['ignore', 'pipe', 'pipe'] })
    ...
    child.on('close', code => {
      ...
      resolve({ content, is_error: code !== 0, newCwd: (newCwd && newCwd !== cwd) ? newCwd : null })
    })
...
async function toolExitPlanMode(db, ctx, input, io) {
  const content = input?.content
  if (!content || typeof content !== 'string' || content.trim().length < 20) {
    return { content: 'ExitPlanMode requires content field with the plan text.', is_error: true }
  }

  // Validate template tags
  const tagCheck = validatePlanTemplateTags(content)
  if (!tagCheck.ok) return { content: tagCheck.reason, is_error: true }

  // Validate Given/When/Then format
  const gwtCheck = validateGivenWhenThen(content)
  if (!gwtCheck.ok) return { content: gwtCheck.reason, is_error: true }

  // Show plan to user for approval (no INSERT yet)
  if (io?.println) io.println(`\n--- Plan for approval ---\n${content}\n--- End plan ---`)
  const answer = io?.ask ? await io.ask('Approve this plan? [y/N] ') : 'y'
  if (!/^y(es)?$/i.test(String(answer || '').trim())) {
    return { content: 'Plan rejected by user. Revise and resubmit.', is_error: true }
  }

  // INSERT and immediately approve in one transaction (only after user approves)
  const hash = randomUUID().slice(0, 16)
  withTransaction(db, () => {
    db.prepare(`INSERT INTO plans(conversation_id, content, hash, status, approved_at) VALUES (?,?,?,'approved',datetime('now'))`).run(ctx.conversationId, content, hash)
  })

  const check = canTransition(db, ctx.conversationId, 'implement')
  if (!check.ok) return { content: check.reason, is_error: true }
  setPhase(db, ctx.conversationId, 'implement')
  return { content: 'Plan approved by user. Phase: implement. Mutating tools unlocked.', is_error: false }
}

/** Validate that each criterion block has Given/When/Then lines. */
function validateGivenWhenThen(content) {
  const lines = content.split('\n')
  ...
  for (const line of lines) {
    ...
    if (inCriteria) {
      if (line.trim().match(/^\d+\.\s+Given\b/i)) {
  ...
  if (!blocks.length) return { ok: false, reason: 'No Given/When/Then criteria found in Success Criteria section.\nEach criterion must start with "Given..." and include "When..." and "Then..." lines.' }

  const invalid = []
  for (const block of blocks) {
    if (!/\bGiven\b/i.test(block)) invalid.push(`Missing Given: ${block.slice(0, 80)}`)
    else if (!/\bWhen\b/i.test(block)) invalid.push(`Missing When: ${block.slice(0, 80)}`)
    else if (!/\bThen\b/i.test(block)) invalid.push(`Missing Then: ${block.slice(0, 80)}`)
  }

  if (invalid.length) return { ok: false, reason: `Plan rejected — incomplete Given/When/Then:\n${invalid.map(i => `  ${i}`).join('\n')}` }
  return { ok: true }
}

/** §4.6 + §8.5.1 — validate that every success criterion has a [template:] tag. */
function validatePlanTemplateTags(content) {
  const VALID_TEMPLATES = new Set(['tool_contract', 'phase_transition', 'hook_contract', 'e2e_workflow', 'process_output', 'file_state', 'http_response'])
  const TAG_RE = /\[template:\s*(\w+)\]/

  // Extract success criteria section — collect multi-line blocks
  const lines = content.split('\n')
  let inCriteria = false
  const criteria = []
  let currentBlock = []
  ...
  for (const line of lines) {
    ...
    if (inCriteria) {
      if (line.trim().match(/^\d+\.\s/)) {
        if (currentBlock.length) criteria.push(currentBlock.join('\n'))
        currentBlock = [line.trim()]
      } else if (currentBlock.length && line.trim()) {
  ...
  if (!criteria.length) return { ok: false, reason: 'Plan has no success criteria section or no numbered criteria found.' }
...
async function toolSubmitImplementation(db, ctx, io) {
  const phase = db.prepare(`SELECT phase FROM conversations WHERE id = ?`).get(ctx.conversationId)?.phase
  if (phase !== 'implement') return { content: `Cannot submit: phase is ${phase}, not implement.`, is_error: true }

  const { spawnSync } = await import('node:child_process')

  // Show diff
  const diff = spawnSync('git', ['diff'], { cwd: ctx.cwd, encoding: 'utf8', timeout: 10_000 })
  const diffText = diff.stdout?.trim() || '(no uncommitted changes)'
  if (io?.println) io.println(`\n--- Changes ---\n${diffText.slice(0, 3000)}${diffText.length > 3000 ? '\n...(truncated)' : ''}\n--- End changes ---`)

  // Ask user to approve
  const answer = io?.ask ? await io.ask('Approve implementation and run tests? [y/N] ') : 'n'
  if (!/^y(es)?$/i.test(String(answer || '').trim())) {
    return { content: 'Implementation not approved. Continue working or revise.', is_error: false }
  }

  // Auto-run tests from plan's Given/When/Then
  const plan = db.prepare(`SELECT content FROM plans WHERE conversation_id = ? AND status = 'approved' ORDER BY id DESC LIMIT 1`).get(ctx.conversationId)
  if (!plan) return { content: 'No approved plan found for test generation.', is_error: true }

  if (io?.println) io.println('Generating and running tests from plan criteria...')
  const { generateAndRunTests } = await import('./testgen.mjs')
  const rt = { sessionId: ctx.sessionId, conversationId: ctx.conversationId, runtimeDbPath: process.env.AGENT_SDLC_DB || '', cwd: ctx.cwd, settings: ctx.settings }
  const results = await generateAndRunTests(db, rt, plan.content, s => { if (io?.println) io.println(s) })

  // Transition to test
  setPhase(db, ctx.conversationId, 'test')

  const passed = results.filter(r => r.passed).length
  const total = results.length
  let resultText = `Tests: ${passed}/${total} passed\n`
  for (const r of results) {
    resultText += `  ${r.passed ? '✓' : '✗'} ${r.criterion.slice(0, 80)}\n`
    if (!r.passed && r.output) resultText += `    ${r.output.split('\n')[0]}\n`
  }

  if (passed === total && total > 0) {
    if (io?.println) io.println(resultText)
    const approveTests = io?.ask ? await io.ask('All tests pass. Approve to complete workflow? [y/N] ') : 'n'
    if (/^y(es)?$/i.test(String(approveTests || '').trim())) {
      setPhase(db, ctx.conversationId, 'verify')

      // Show coverage report
      if (io?.println) {
        io.println('Coverage Report:')
        for (const r of results) io.println(`  ${r.passed ? '✓' : '✗'} ${r.criterion}`)
        io.println(`\nAggregate: ${passed}/${total} passed`)
      }

      const approveFinal = io?.ask ? await io.ask('Approve and commit? [y/N] ') : 'n'
      if (/^y(es)?$/i.test(String(approveFinal || '').trim())) {
        setPhase(db, ctx.conversationId, 'done')
        // Auto-commit
        const planTitle = plan.content.split('\n').find(l => l.startsWith('#'))?.replace(/^#+\s*/, '') || 'SDLC workflow complete'
        spawnSync('git', ['add', '-A'], { cwd: ctx.cwd, encoding: 'utf8' })
        const commit = spawnSync('git', ['commit', '-m', `${planTitle}\n\nPlan approved and verified via ona SDLC workflow.`], { cwd: ctx.cwd, encoding: 'utf8', timeout: 30_000 })
        const hash = commit.status === 0 ? spawnSync('git', ['rev-parse', '--short', 'HEAD'], { cwd: ctx.cwd, encoding: 'utf8' }).stdout?.trim() : null
        return { content: `${resultText}\nWorkflow complete. Phase: done.${hash ? ` Committed: ${hash}` : ''}`, is_error: false }
      }
      return { content: `${resultText}\nPhase: verify. Review coverage and approve when ready.`, is_error: false }
    }
    return { content: `${resultText}\nPhase: test. All tests pass — waiting for approval.`, is_error: false }
  }

  return { content: `${resultText}\nPhase: test. ${total - passed} test(s) failing. Fix and resubmit.`, is_error: false }
}
...
function toolSkill(cwd, input) {
  const name = input?.skill
  if (!name) return { content: 'Skill: missing skill name', is_error: true }

  // Search for skill markdown file in standard locations:
  // .ona/skills/<name>/SKILL.md, .claude/skills/<name>/SKILL.md,
  // .ona/skills/<name>.md, .claude/skills/<name>.md
  const searchDirs = [
    path.join(cwd, '.ona', 'skills'),
    path.join(cwd, '.claude', 'skills'),
    path.join(process.env.HOME || '', '.ona', 'skills'),
  ...
  for (const dir of searchDirs) {
    ...
    const dirSkill = path.join(dir, name, 'SKILL.md')
    if (fs.existsSync(dirSkill)) {
      try {
        const content = fs.readFileSync(dirSkill, 'utf8')
        return { content: `Skill '${name}':\n\n${content}`, is_error: false }
      } catch (e) {
        return { content: `Skill: failed to read ${dirSkill}: ${e.message}`, is_error: true }
      }
    }
    // Flat format: <name>.md
    const flatSkill = path.join(dir, `${name}.md`)
    if (fs.existsSync(flatSkill)) {
      try {
        const content = fs.readFileSync(flatSkill, 'utf8')
        return { content: `Skill '${name}':\n\n${content}`, is_error: false }
      } catch (e) {
        return { content: `Skill: failed to read ${flatSkill}: ${e.message}`, is_error: true }
      }
    }
  }

  // No skill file found — return available skills list
  const available = []
  for (const dir of searchDirs) {
    if (!fs.existsSync(dir)) continue
    try {
      ...
      for (const e of entries) {
        if (e.isDirectory() && fs.existsSync(path.join(dir, e.name, 'SKILL.md'))) {
          ...
          available.push(e.name.replace(/\.md$/, ''))
        }
      }
  ...
  if (available.length) {
    return { content: `Skill '${name}' not found. Available skills: ${[...new Set(available)].join(', ')}`, is_error: true }
  }
  return { content: `Skill '${name}' not found. No skills directory configured. Create .ona/skills/${name}/SKILL.md to define a skill.`, is_error: true }
}
...
export function anthropicToolDefinitions() {
  return [
    ...
    { name: 'SubmitImplementation', description: 'Submit implementation for review. Shows diff, runs tests automatically on approval.', input_schema: { type: 'object', properties: {} } },
    { name: 'EnterPlanMode', description: 'Enter SDLC planning phase (§8).', input_schema: { type: 'object', properties: {} } },
    { name: 'ExitPlanMode', description: 'Submit plan for approval. Plan must have Given/When/Then criteria with [template:] tags.', input_schema: { type: 'object', properties: { content: { type: 'string', description: 'The full plan text with ## Success Criteria containing Given/When/Then blocks and [template:] tags' } }, required: ['content'] } },

...
import { setPhase, getPhase } from './workflow.mjs'
...
// Tools the model cannot call during planning phase
const PLANNING_BLOCKED_TOOLS = new Set(['Write', 'Edit', 'Bash', 'NotebookEdit'])
...
export async function executeBuiltinTool(db, ctx, toolName, toolInput, io) {
  ...
  // The harness owns phase-based tool filtering. If a model calls a blocked tool
  // anyway (e.g. hallucination), enforce it here as a hard stop.
  const phase = getPhase(db, conversationId)
  if (phase === 'planning' && PLANNING_BLOCKED_TOOLS.has(toolName)) {
    return { content: `Tool ${toolName} is not available in planning phase. Use Read, Glob, or Grep to explore the codebase, then write your plan as text.`, is_error: true }
  }
  ...
  try {
    switch (toolName) {
      ...
      // Phase transitions are owned by the orchestrator harness, not the model.
      // If the model somehow calls these, return a clear error.
      case 'EnterPlanMode':
      case 'ExitPlanMode':
      case 'SubmitImplementation':
      case 'SubmitTest':
        return { content: `[SDLC] ${toolName} is not a model tool. Phase transitions are managed by the harness.`, is_error: true }
...
// Strip BOS/EOS tokens that some models (DeepSeek, Qwen) leak into tool call arguments
function sanitizeBriefMessage(msg) {
  return msg
...
function toolBrief(io, input) {
  const raw = input?.message || input?.content || ''
  const msg = sanitizeBriefMessage(raw)

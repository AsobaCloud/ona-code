...
async function toolExitPlanMode(db, ctx, input, io) {
  ...
  withTransaction(db, () => {
    db.prepare(`INSERT INTO plans(conversation_id, content, hash, status) VALUES (?,?,?,'draft')`).run(ctx.conversationId, content, hash)
  })
  ...
  withTransaction(db, () => {
    const plan = db.prepare(`SELECT id FROM plans WHERE conversation_id = ? ORDER BY id DESC LIMIT 1`).get(ctx.conversationId)
    if (plan) db.prepare(`UPDATE plans SET status = 'approved', approved_at = datetime('now') WHERE id = ?`).run(plan.id)
  })

...
export function saveSecureCredentials({ apiKey, bearerToken, zhipuApiKey }) {
  ...
  if (zhipuApiKey !== undefined) { if (zhipuApiKey === '') delete next.zhipuApiKey; else next.zhipuApiKey = zhipuApiKey }
  fs.writeFileSync(p, JSON.stringify(next, null, 0), { mode: 0o600 })
}

export function resolveZhipuApiKey() {
  const fromEnv = nonempty(process.env.ZAI_API_KEY)
  if (fromEnv) return fromEnv
  const file = readSecureAuthFile()
  return nonempty(file?.zhipuApiKey) || ''
}
...
function readSecureAuthFile() {
  try {
    ...
    return { apiKey: nonempty(j.apiKey), bearerToken: nonempty(j.bearerToken), zhipuApiKey: nonempty(j.zhipuApiKey) }
  } catch { return null }
}
